...
def create_consensus_prompt(query, round1_responses, round2_responses):
    return f"""You are the Meta-Consensus System analyzing outputs from three AI perspectives.
...
def run_multistage_debate(query):
    ...
    round1_logic = get_gemini_response(logic_prompt)          # Logic = Gemini
    round1_ethics = get_anthropic_response(ethics_prompt)     # Ethics = Claude
    round1_intuition = get_openai_response(intuition_prompt)  # Intuition = GPT
    ...
    round2_logic = get_gemini_response(logic_rebuttal_prompt)
    ...
    round2_intuition = get_openai_response(intuition_rebuttal_prompt)
    ...
    try:
        ...
        r1_vecs = [model.encode(r, convert_to_tensor=True)
                   for r in round1_responses.values() if not r.startswith("[")]
        r2_vecs = [model.encode(r, convert_to_tensor=True)
                   for r in round2_responses.values() if not r.startswith("[")]
        r1_sim = calculate_similarity(r1_vecs) if len(r1_vecs) >= 2 else None
        r2_sim = calculate_similarity(r2_vecs) if len(r2_vecs) >= 2 else None
        convergence = (r2_sim - r1_sim) if r1_sim and r2_sim else None
        if r1_sim: print(f"Round 1 Similarity: {r1_sim:.3f}")
        if r2_sim: print(f"Round 2 Similarity: {r2_sim:.3f}")
        if convergence: print(f"Convergence Rate: {convergence:+.3f}")
    except Exception as e:
        print(f"⚠️  Similarity analysis failed: {str(e)[:100]}")
        r1_sim = r2_sim = convergence = None
    ...
    print("\nResults saved to results/multistage_debate.json")
